...
const modes: Omit<Record<GameMode, { en: [string, string]; ru: [string, string] }>, 'swordDuel'> = {
  ...
  craftSurvival: { en: ['Craft Survival', 'Weapons unlock after the peace phase. Loot position first, fight later.'], ru: ['Майн выживание', 'Оружие включается только после мирной фазы: сначала займи позицию, потом дерись.'] },
